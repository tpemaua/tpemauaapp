...
export class NewscheduleComponent {
      ...
      pontos_hora.map(p => {
        ...
        while (contador < 10) {
            ...
            if (p.pubs[s].userId == undefined) {
              ...
              for (let g = 0; g < this.usersesc.length; g++) {
                ...
                if (s > 0 && idade == 17) {
                  ...
                  try {
                    if (
                      p.pubs[s - 1].congregation["nome"] !=
                      this.usersesc[g].congregation["nome"]
                    ) {
                      continue;
                    }
                  } catch (e) {
                ...
                if (s > 0) {
                  ...
                  if (
                    casal == false &&
                    p.pubs[s - 1].sex != this.usersesc[g].sex
                  )
                    continue;
                }
                /*     if (s > 0) {
          ...
          if (p.pubs[0].userId != undefined && p.pubs[1].userId == undefined) {
            ...
            for (let s = 0; s < p.npubs; s++) {
              if (p.pubs[s].userId == undefined) {
                ...
                for (let g = 0; g < this.usersesc.length; g++) {
                  ...
                  for (let m = 0; m < this.mesmasemana_agenda.length; m++) {
                    if (
                      this.mesmasemana_agenda[m].id ==
                        this.usersesc[g].userId &&
                      this.mesmasemana_agenda[m].dia == dia1formatadopre
                    ) {
                      semanaigualpre = true;
                      break;
            ...
            if (
              p.pubs[0].userId != undefined &&
              p.pubs[1].userId == undefined
            ) {
              for (let g = 0; g < this.usersesc.length; g++) {
                if (p.pubs[0].userId == this.usersesc[g].userId) {
                  let useralone = this.usersesc.splice(g, 1);
  ...
  geraEscala() {
    ...
    while (this.diaatual < moment.utc(this.validity.end)) {
      ...
      if (this.buscaFeriado()) this.dayoption = 7;
      else this.dayoption = this.diaatual.day();
      // regra feriado fim
      this.pontos_dia = this.pontos.filter(
  ...
  deletaEscala() {
    let resposta = prompt(
      "Tem certeza que quer deletar a Escala? Digite: deletar"
    );
    if (resposta == "deletar") {
      if (this.escala.length > 0) {
        ...
        if (this.escala.length > 0) {
          ...
          this.authService.getAllLeds(this.datainicio).subscribe(
            leds => {
              ...
              for (let t = 0; t < this.users.length; t++) {
                let user = this.reportcount.filter(
              ...
              for (let u = 0; u < this.escala[i].pontos[p].length; u++) {
                ...
                for (let s = 0; s < this.escala[i].pontos[p][u].npubs; s++) {
                  if (
                    ...
                    console.log(
                      `led ${this.escala[i].pontos[p][u].pubs[s].firstName} modificado ${data.sim} ${data.nao}`
                    );
                    led = true;
                    break;
                  }
              ...
              for (let u = 0; u < this.escala[i].pontos[p].length; u++) {
                ...
                for (let s = 0; s < this.escala[i].pontos[p][u].npubs; s++) {
                  if (
                    ...
                    console.log(
                      `sub ${this.escala[i].pontos[p][u].pubs[s].firstName} modificado ${data.sim} ${data.nao}`
                    );
                    led = true;
                    break;
                  }
...
/** @nocollapse */
NewscheduleComponent.ctorParameters = () => [{ type: AuthService }];
